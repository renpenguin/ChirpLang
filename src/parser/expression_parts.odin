...
// Attempt to build a `FunctionCall` from the passed location in the TokenStream. `char_index` should point to the potential function name
build_function :: proc(
	func_name: NameReference,
	...
	if char_index^ + 1 >= len(tokens) || tokens[char_index^ + 1] != t.Token(t.Bracket{.Round, .Opening}) do return
	ok = true
	...
	was_comma := true
	for was_comma {
		if tokens[char_index^ + 1] == t.Token(t.Bracket{.Round, .Closing}) {char_index^ += 1;break}
